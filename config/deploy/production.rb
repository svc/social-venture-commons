...
namespace :deploy do
  task :start, :roles => [:app] do
    run "cd #{deploy_to}/current && nohup thin -C config/thin_production_config.yml start"
  end
 
  task :stop, :roles => [:app] do
    run "cd #{deploy_to}/current && nohup thin -C config/thin_production_config.yml stop"
  end
 
  task :restart, :roles => [:app] do
    run "cd #{deploy_to}/current && nohup thin -C config/thin_production_config.yml restart"
  end
